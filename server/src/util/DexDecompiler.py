...
class DexDecompiler:
    ...
    args_30t = BitStruct(None,
                         Octet('op'),
                         Const(Octet(None), 0),
                         BitField('AAAAAAAA', 32, bytesize=8,
                                  signed=True, swapped=True),
                         )
    ...
    args_31t = BitStruct(None,
                         Octet('op'),
                         Octet('AA'),
                         BitField('BBBBBBBB', 32, bytesize=8,
                                  signed=True, swapped=True),
                         )
    ...
    args_51l = BitStruct(None,
                         Octet('op'),
                         Octet('AA'),
                         BitField('BBBBBBBBBBBBBBBB', 32,
                                  bytesize=8, swapped=True)
                         )
    ...
    def extract_instruction(self):
        try:
            ...
            if (sw_op == 0x00) and self.RawBytes[self.pos+1] == 0x01:
                ...
                fadp = self.FillArrayDataPayload.parse(
                    self.RawBytes[self.pos:])
                self.pos += int((fadp['sz'] *
                                fadp['element_width'] + 1)/2 + 4)*2
                operation = dict(fadp)
            else:
    ...
    def decode_10t_args(self, op):
        ...
        offset = a10t['AA']
        args = ' {0: 05x} // {1:+05x}'.format(
            (self.pos >> 1) - 1 + offset, offset)
        return args
    ...
    def decode_20t_args(self, op):
        ...
        offset = a20t['AAAA']
        args = ' {0: 05x} // {1:+05x}'.format(
            (self.pos >> 1) - 2 + offset, offset)
        return args

    def decode_30t_args(self, op):
        a30t = self.args_30t.parse(op)
        offset = a30t['AAAAAAAA']
        args = ' {0: 09x} // {1:+09x}'.format(
            (self.pos >> 1) - 3 + offset, offset)
        return args
    ...
    def decode_21t_args(self, op):
        ...
        offset = a21t['BBBB']
        args = ' v{0:d}, {1: 05x} // {2:+05x}'.format(
            a21t['AA'], (self.pos >> 1) - 2 + offset, offset)
        return args
    ...
    def decode_21s_args(self, op):
        ...
        args = ' v{0:d}, #int {1:d} // #{1: 05x}'.format(
            a21s['AA'], a21s['BBBB'])
        return args
    ...
    def decode_22t_args(self, op):
        ...
        offset = a22t['CCCC']
        args = ' v{0:d}, v{1:d}, {2: 05x} // {3:+05x}'.format(
            a22t['A'], a22t['B'], (self.pos >> 1) - 2 + offset, offset)
        return args
    ...
    def decode_f21c_args(self, op):
        ...
        args = ' v{0:d}, {1} [{2}].{3} // field@{4:04x}'.format(
            a21c['AA'], f['type'], f['class'], f['name'], a21c['BBBB'])
        return args
    ...
    def decode_t21c_args(self, op):
        ...
        typen = '{type_name}'
        args = ' v{0:d}, [{1}] // type@{2:04x}'.format(
            a21c['AA'], typen, a21c['BBBB'])
        return args
    ...
    def decode_s21c_args(self, op):
        ...
        st = '{string}'
        args = ' v{0:d}, [{1}] // string@{2:04x}'.format(
            a21c['AA'], st, a21c['BBBB'])
        return args
    ...
    def decode_s31c_args(self, op):
        ...
        st = '{string}'
        args = ' v{0:d}, [{1}] // string@{2:08x}'.format(
            a31c['AA'], st, a31c['BBBBBBBB'])
        return args
    ...
    def decode_f22c_args(self, op):
        ...
        f = {'type': '{type}', 'class': '{class}', 'name': '{field}'}
        args = ' v{0:d}, v{1:d}, {2} [{3}].{4} // field@{5:04x}'.format(
            a22c['A'], a22c['B'], f['type'], f['class'], f['name'], a22c['CCCC'])
        return args
    ...
    def decode_t22c_args(self, op):
        ...
        typen = '{type_name}'
        args = ' v{0:d}, v{1:d}, [{2}] // type@{3:04x}'.format(
            a22c['A'], a22c['B'], typen, a22c['CCCC'])
        return args

    def decode_22s_args(self, op):
        a22s = self.args_22s.parse(op)
        args = ' v{0:d}, v{1:d}, #int {2:d} // #{2: 05x}'.format(
            a22s['A'], a22s['B'], a22s['CCCC'])
        return args

    def decode_22b_args(self, op):
        a22b = self.args_22b.parse(op)
        args = ' v{0:d}, v{1:d}, #int {2:d} // #{2: 03x}'.format(
            a22b['AA'], a22b['BB'], a22b['CC'])
        return args
    ...
    def decode_i35c_args(self, op):
        ...
        if (a35c['A'] == 0):
            ...
            args += 'v{0:d}, v{1:d} @{2:d}'.format(
                a35c['C'], a35c['D'], a35c['BBBB'])
        elif (a35c['A'] == 3):
            args += 'v{0:d}, v{1:d}, v{2:d} @{3:d}'.format(
                a35c['C'], a35c['D'], a35c['E'], a35c['BBBB'])
        elif (a35c['A'] == 4):
            args += 'v{0:d}, v{1:d}, v{2:d}, v{3:d} @{4:d}'.format(
                a35c['C'], a35c['D'], a35c['E'], a35c['F'], a35c['BBBB'])
        elif (a35c['A'] == 5):
            args += 'v{0:d}, v{1:d}, v{2:d}, v{3:d}, v{4:d} @{5:d}'.format(
                a35c['C'], a35c['D'], a35c['E'], a35c['F'], a35c['G'], a35c['BBBB'])
        return args + method
    ...
    def decode_t35c_args(self, op):
        ...
        if (a35c['A'] == 0):
            ...
            args += 'v{0:d}, [{1}] // type@{2:04x}'.format(
                a35c['C'], typen, a35c['BBBB'])
        elif (a35c['A'] == 2):
            args += 'v{0:d}, v{1:d}, [{2}] // type@{3:04x}'.format(
                a35c['C'], a35c['D'], typen, a35c['BBBB'])
        elif (a35c['A'] == 3):
            args += 'v{0:d}, v{1:d}, v{2:d}, [{3}] // type@{4:04x}'.format(
                a35c['C'], a35c['D'], a35c['E'], typen, a35c['BBBB'])
        elif (a35c['A'] == 4):
            args += 'v{0:d}, v{1:d}, v{2:d}, v{3:d}, [{4}] // type@{5:04x}'.format(
                a35c['C'], a35c['D'], a35c['E'], a35c['F'], typen, a35c['BBBB'])
        elif (a35c['A'] == 5):
            args += 'v{0:d}, v{1:d}, v{2:d}, v{3:d}, v{4:d}, [{5}] // type@{6:04x}'.format(
                a35c['C'], a35c['D'], a35c['E'], a35c['F'], a35c['G'], typen, a35c['BBBB'])
        return args
    ...
    def decode_operation(self, op):
        if type(op) == bytes:
            if op[0] in InstructionSet:
                method = getattr(
                    self, 'decode_'+InstructionSet[op[0]][1]+'_args')
                if not method:
    ...
    def disassembleBytes(self, b: bytes) -> str:
        self.load_bytearray(b)
